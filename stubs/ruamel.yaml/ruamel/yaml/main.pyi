...
# YAML.load() can produce arbitrary objects through tags and registered constructors, while dump() accepts arbitrary
# Python objects through registered representers; use one documented Any alias for these distinct boundaries.
...
class YAML:
    ...
    def load(self, stream: Path | _ReadStream) -> _YAMLObject: ...
    def load_all(self, stream: Path | _ReadStream) -> Iterator[_YAMLObject]: ...
    ...
    # stream cannot be None here because context-manager calls are type-checked by _YAMLContext instead.
    def dump(self, data: _YAMLObject, stream: Path | _WriteStream, *, transform: Callable[[str], str] | None = None) -> None: ...
    def dump_all(
        self, documents: Iterable[_YAMLObject], stream: Path | _WriteStream, *, transform: Callable[[str], str] | None = None
    ) -> None: ...
    def Xdump_all(
        self, documents: Iterable[_YAMLObject], stream: Path | _WriteStream, *, transform: Callable[[str], str] | None = None
    ) -> None: ...
    ...
    @overload
    def map(self) -> dict[_YAMLObject, _YAMLObject]: ...
    @overload
    def map(self, **kw: _T) -> dict[str, _T]: ...

    @overload
    def seq(self) -> list[_YAMLObject]: ...
    @overload
...
class _RoundTripYAML(YAML):
    ...
    @overload
    def map(self) -> CommentedMap[_YAMLObject, _YAMLObject]: ...
    @overload
    def map(self, **kw: _T) -> CommentedMap[str, _T]: ...

    @overload
    def seq(self) -> CommentedSeq[_YAMLObject]: ...
    @overload
...
class _YAMLContext(YAML):
    def dump(self, data: _YAMLObject, stream: Unused = None, *, transform: None = None) -> None: ...  # type: ignore[override]
...
class YAMLContextManager:
    ...
    def init_output(self, first_data: _YAMLObject) -> None: ...
    def dump(self, data: _YAMLObject) -> None: ...
...
class YAMLObjectMetaclass(type):
    def __init__(cls, name: str, bases: tuple[type, ...], kwds: dict[str, object], /) -> None: ...

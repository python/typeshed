...
@type_check_only
class _UnknownAsyncBinaryIO(AsyncBase[bytes], Generic[_NameT]):
...
class AsyncBufferedIOBase(_UnknownAsyncBinaryIO[_NameT]):
    async def read1(self, size: int = ..., /) -> bytes: ...
    def detach(self) -> FileIO: ...
    @property
    def raw(self) -> FileIO: ...

class AsyncIndirectBufferedIOBase(AsyncIndirectBase[bytes], _UnknownAsyncBinaryIO[_NameT], Generic[_NameT]):
    async def read1(self, size: int = ..., /) -> bytes: ...
    def detach(self) -> FileIO: ...
    @property
    def raw(self) -> FileIO: ...

class AsyncBufferedReader(AsyncBufferedIOBase[_NameT]):
    async def peek(self, size: int = ..., /) -> bytes: ...

class AsyncIndirectBufferedReader(AsyncIndirectBufferedIOBase[_NameT]):
    async def peek(self, size: int = ..., /) -> bytes: ...

class AsyncFileIO(_UnknownAsyncBinaryIO[_NameT]):
    async def readall(self) -> bytes: ...

class AsyncIndirectFileIO(AsyncIndirectBase[bytes], _UnknownAsyncBinaryIO[_NameT], Generic[_NameT]):
    async def readall(self) -> bytes: ...

...
class Writer(_html_base.Writer):
    supported: ClassVar[tuple[Literal["html5"], Literal["xhtml"], Literal["html"]]]
...
class HTMLTranslator(_html_base.HTMLTranslator):
    ...
    def visit_inline(self, node):
        ...
        if (isinstance(node.parent, nodes.literal_block) and "code" in node.parent.get("classes")) or (
            isinstance(node.parent, nodes.literal) and getattr(node.parent, "html5tagname", None) == "code"
        ):
            if classes == ["ln"]:
    ...
    def section_title_tags(self, node):
        ...
        if ids and getattr(self.settings, "section_self_link", None) and not isinstance(node.parent, nodes.document):
            self_link = f'<a class="self-link" title="link to this section" href="#{ids[0]}"></a>'

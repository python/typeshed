...
def check_new_syntax(tree: ast.AST, path: Path) -> list[str]:
    ...
    class IfFinder(ast.NodeVisitor):
        def visit_If(self, node: ast.If) -> None:
            if isinstance(node.test, ast.Compare) and node.orelse and ast.unparse(node.test).startswith("sys.version_info < "):
                new_syntax = ast.unparse(node.test).replace("<", ">=", 1)
                errors.append(
                    f"{path}:{node.lineno}: When using if/else with sys.version_info, put the code for new Python versions first, e.g. `{new_syntax}`"
                )
            self.generic_visit(node)

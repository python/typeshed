...
class UserDict(MutableMapping[_KT, _VT]):
    data: dict[_KT, _VT]
    # __init__ should be kept in line with `dict.__init__`, which has the same semantics
    @overload
    def __init__(self: UserDict[_KT, _VT]) -> None: ...
    @overload
    def __init__(self: UserDict[str, _VT], **kwargs: _VT) -> None: ...
    @overload
    ...
    # Next overload is for UserDict(string.split(sep) for string in iterable)
    # Cannot be Iterable[Sequence[_T]] or otherwise UserDict(["foo", "bar", "baz"]) is not an error
    @overload

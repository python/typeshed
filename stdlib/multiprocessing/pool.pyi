...
class ApplyResult(Generic[_T]):
    if sys.version_info >= (3, 8):
        def __init__(
            self, pool: Pool, callback: Callable[[_T], Any] | None, error_callback: Callable[[BaseException], Any] | None
        ) -> None: ...
    else:
        def __init__(
            self,
            cache: dict[int, ApplyResult[Any]],
            callback: Callable[[_T], Any] | None,
...
class MapResult(ApplyResult[list[_T]]):
    if sys.version_info >= (3, 8):
        def __init__(
            ...
            chunksize: int,
            length: int,
            callback: Callable[[list[_T]], Any] | None,
            error_callback: Callable[[BaseException], Any] | None,
        ) -> None: ...
    else:
        def __init__(

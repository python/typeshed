...
class Future(Generic[_T_co]):
    ...
    def done(self) -> bool: ...
    def add_done_callback(self, fn: Callable[[Future[_T_co]], object]) -> None: ...
    def result(self, timeout: float | None = None) -> _T_co: ...
    def set_running_or_notify_cancel(self) -> bool: ...
    def set_result(self, result: _T_co) -> None: ...
...
class Executor:
    if sys.version_info >= (3, 9):
        def submit(self, __fn: Callable[_P, _T_co], *args: _P.args, **kwargs: _P.kwargs) -> Future[_T_co]: ...
    else:
        def submit(self, fn: Callable[_P, _T_co], *args: _P.args, **kwargs: _P.kwargs) -> Future[_T_co]: ...

    def map(
        self, fn: Callable[..., _T_co], *iterables: Iterable[Any], timeout: float | None = None, chunksize: int = 1
    ) -> Iterator[_T_co]: ...
    if sys.version_info >= (3, 9):
...
def as_completed(fs: Iterable[Future[_T_co]], timeout: float | None = None) -> Iterator[Future[_T_co]]: ...

class DoneAndNotDoneFutures(NamedTuple, Generic[_T_co]):
    done: set[Future[_T_co]]
    not_done: set[Future[_T_co]]

def wait(
    fs: Iterable[Future[_T_co]], timeout: float | None = None, return_when: str = "ALL_COMPLETED"
) -> DoneAndNotDoneFutures[_T_co]: ...

...
configuration file. This means that you don't _need_ to do anything yourself to
run the code formatters. When you push a commit, a bot will run those for you
right away and add a commit to your PR.

That being said, if you _want_ to run the checks locally when you commit,
...
_Note:_ In its current implementation, typeshed cannot contain stubs for
...
Accepted features that _cannot_ yet be used in typeshed include:
...
Stub files are _like_ Python files and you should generally expect them
